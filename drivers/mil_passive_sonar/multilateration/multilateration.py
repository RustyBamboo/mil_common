...
def get_time_delta(t, non_ref, ref):
    ...
    if len(non_ref) != len(t):
        raise RuntimeError(
            'Vector of time values must have the same size as the signals')
...
class Multilaterator(object):
    ...
    def __init__(self, receiver_locations, c,
                 method):  # speed in millimeters/microsecond
        '''
        ...
        self.method = method
        self.solvers = {
            'bancroft': self.estimate_pos_bancroft,
    ...
    def get_pulse_location(self, dtoa, method=None):
        ...
        if not len(self.receiver_locations) == len(dtoa):
            raise RuntimeError(
                'Number of non-reference receivers and dtoa measurents don\'t match'
            )
    ...
    def estimate_pos_bancroft(self, dtoa):
        ...
        def get_B(delta):
            ...
            for i in xrange(N):
                B[i] = np.concatenate(
                    [self.receiver_locations[i] /
                     (self.c), [-dtoa[i]]]) + delta
            return B

        delta = min(
            [.1 * np.random.randn(4) for i in xrange(10)],
        ...
        Lambdas = quadratic(L(Bpe, Bpe), 2 * (L(Bpa, Bpe) - 1), L(Bpa, Bpa))
        if not Lambdas:
            return [0, 0, 0]
    ...
    def estimate_pos_LS(self, dtoa, cost_func):
        ...
        result = optimize.minimize(
            cost_func, init_guess, method=opt_method, options=opt, tol=1e-15)
        if (result.success):
    ...
    def cost_LS1(self, potential_pulse):
        ...
        for pair in self.pairs:
            cost += (np.linalg.norm(x - rcv[pair[0]]) -
                     np.linalg.norm(x - rcv[pair[1]]) - c *
                     (t[pair[0]] - t[pair[1]]))**2
        return cost
    ...
    def listen(self, pulse):
        ...
        for idx in range(self.n - 1):
            src_range = np.sqrt(
                sum(
                    np.square(
                        pulse.position() - self.receiver_locations[idx])))
